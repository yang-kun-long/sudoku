...
const techniques = [
  ...
  },
  {
    ...
    rule: '本例证明第 1 行第 1 列的 9 和第 5 行第 3 列的 9 至少有一个成立，所以同时看到这两个端点的格可以删除 9。',
    steps: ['端点先固定为候选 9：第 1 行第 1 列与第 5 行第 3 列', '同一个双候选格内部可以有强关系，例如第 1 行第 1 列的 9 和 2', '同一区域里某个候选只剩两个位置时，也可以有格间强关系，例如第 2 行的两个 5', '虚线跨格连接同一个候选，表示两个互相看见的位置不能同时取这个候选', '最后删除同时看到两个 9 端点的目标格候选 9'],
    links: [
      ...
      { from: 15, fromCandidate: 5, to: 10, toCandidate: 5, relation: 'strong' },
      { from: 10, fromCandidate: 5, to: 10, toCandidate: 9, relation: 'strong' },
      { from: 10, fromCandidate: 9, to: 38, toCandidate: 9, relation: 'weak' },
    ],
    terms: [
      { name: '端点', description: '第 1 行第 1 列的 9 与第 5 行第 3 列的 9。链证明两者至少有一个成立。' },
      { name: '格内强关系', description: '同一个双候选格只有两个可能数字，不是其中一个就必须是另一个。' },
      { name: '格间强关系', description: '第 2 行的候选 5 只出现在第 2 行第 7 列和第 2 行第 2 列，所以这两个 5 至少有一个成立。' },
      { name: '弱关系', description: '两个互相看见的位置共享同一个候选时，不能同时取这个候选。' },
      { name: '删除目标', description: '第 1 行第 3 列同时看到两个 9 端点，因此可以排除 9。' },
    ],
    cells: [
      { index: 0, candidates: [2, 9], key: [2, 9], role: 'chain-node' },
      { index: 7, candidates: [1, 2], key: [1, 2], role: 'chain-node' },
      { index: 24, candidates: [1, 7], key: [1, 7], role: 'chain-node' },
      { index: 15, candidates: [5, 7], key: [5, 7], role: 'chain-node' },
      { index: 10, candidates: [5, 9], key: [5, 9], role: 'chain-node' },
      { index: 38, candidates: [4, 8, 9], key: [9], role: 'chain-node' },
