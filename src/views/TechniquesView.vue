...
const techniques = [
  ...
  },
  {
    ...
    level: '专家',
    summary: '链技巧不是看一堆格子，而是按箭头追踪候选数字之间的关系。实线是强关系：两端至少一个成立；虚线是弱关系：两端不能同时成立。',
    rule: '本例证明第 1 行第 1 列的 9 和第 1 行第 8 列的 9 至少有一个成立，所以同一行里同时看到它们的格可以删除 9。',
    steps: ['端点先固定为候选 9：第 1 行第 1 列与第 1 行第 8 列', '格间强关系：某个候选在同一行、列或宫只剩两个位置时，两端至少一个成立', '格内强关系：同一个双候选格不是其中一个候选，就必须是另一个候选', '弱关系：两个互相看见的位置不能同时取同一个候选', '按实线、虚线交替读完后，删除同时看到两个 9 端点的目标格候选 9'],
    links: [
      { from: 0, fromCandidate: 9, to: 36, toCandidate: 9, relation: 'strong' },
      { from: 36, fromCandidate: 9, to: 38, toCandidate: 9, relation: 'weak' },
      { from: 38, fromCandidate: 9, to: 38, toCandidate: 4, relation: 'strong' },
      { from: 38, fromCandidate: 4, to: 11, toCandidate: 4, relation: 'weak' },
      { from: 11, fromCandidate: 4, to: 16, toCandidate: 4, relation: 'strong' },
      { from: 16, fromCandidate: 4, to: 7, toCandidate: 4, relation: 'weak' },
      { from: 7, fromCandidate: 4, to: 7, toCandidate: 9, relation: 'strong' },
    ],
    terms: [
      { name: '端点', description: '第 1 行第 1 列的 9 与第 1 行第 8 列的 9。链证明两者至少有一个成立。' },
      { name: '格间强关系', description: '第 1 列的候选 9 只出现在第 1 行第 1 列和第 5 行第 1 列，所以这两个 9 至少有一个成立。第 2 行的两个 4 也是同理。' },
      { name: '格内强关系', description: '同一个双候选格只有两个可能数字，不是其中一个就必须是另一个。' },
      { name: '弱关系', description: '两个互相看见的位置共享同一个候选时，不能同时取这个候选。' },
      { name: '删除目标', description: '第 1 行第 3 列同时看到两个 9 端点，因此可以排除 9。' },
    ],
    cells: [
      { index: 0, candidates: [2, 9], key: [9], role: 'chain-node' },
      { index: 36, candidates: [7, 9], key: [9], role: 'chain-node' },
      { index: 38, candidates: [4, 9], key: [4, 9], role: 'chain-node' },
      { index: 11, candidates: [4, 6], key: [4], role: 'chain-node' },
      { index: 16, candidates: [4, 8], key: [4], role: 'chain-node' },
      { index: 7, candidates: [4, 9], key: [4, 9], role: 'chain-node' },
      { index: 2, candidates: [3, 6, 9], eliminate: [9], role: 'target' },
    ],
  },
  {
    id: 'xyz-wing',
